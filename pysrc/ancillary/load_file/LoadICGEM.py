...
def load_HUSTGRACE(*filepath, key: str, lmax: int, lmcs_in_queue=None, get_dates=False, daylist=None):
    """
    This 'load' is to load the file format like -yyyymm.gfc, HUST
    :param filepath: path of SH file
    :param key: '' if there is not any key.
    :param lmax: max degree and order.
    :param lmcs_in_queue: iter, Number of columns where degree l, order m, coefficient clm, and slm are located.
...
def load_ITSG(*filepath, key: str, lmax: int, lmcs_in_queue=None, get_dates=False, daylist=None):
    """
    This 'load' is to load the file format like _yyyy-mm.gfc, including ITSG,AIUB-G3P,Tongji
...
def load_SLR_AIUB(*filepath, key: str, lmax: int, lmcs_in_queue=None, get_dates=False, daylist=None):
    """
    This 'load' is to load the file format like _yymm_1010.gfc, including AIUB-SLR
    ...
    if len(filepath) == 1:
        ...
        if filepath[0].is_file():
            ...
            files_to_load = []
            for i in range(len(file_list)):
                index = file_list[i].name.split('_')[-2].split('.')[0]
                # print(f"file_list[i] name is:{index},{type(index)}")
                # print(daylist)
                # print(index in daylist)
                if index in daylist:
...
def load_SWPU(*filepath, key: str, lmax: int, lmcs_in_queue=None, get_dates=False, daylist=None):
    """
    This 'load' is to load the file format like _yyyymm_lmax.gfc, including SWPU
...
def demo2():
    import pandas as pd
    date_range = pd.date_range(start='2003-01-01',end='2003-03-31',freq='MS').strftime("%y%m").tolist()
    print(date_range)
