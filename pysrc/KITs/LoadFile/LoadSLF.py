...
def demo():
    ...
    filepath = FileTool.get_project_dir("J:\Data_GRACE\Sea Level Fingerprint\SLFsh_coefficients/UTCSR\CF\WITHrotation/")
    begin_date, end_date = date(2002, 4, 1), date(2016, 8, 31)
    CS_Input = LoadSLF().get_CS(filepath,begin_date=begin_date, end_date=end_date,lmcs_in_queue=np.array([0,1,2,4]))
    CS_SLC = LoadSLF().get_CS(filepath,begin_date=begin_date, end_date=end_date,lmcs_in_queue=np.array([0,1,6,8]))

    print(CS_Input.value.shape,CS_SLC.value.shape)
def demo2():
    file_path = "D:\Data_GRACE\Sea Level Fingerprint/"
    print(file_path)
