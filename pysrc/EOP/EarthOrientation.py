...
class GRACE_Exciatation:
    ...
    def I_Matrix_Term(self, mask=None):
        ...
        I_21S = GRD(grid=CoreI21S, lat=self.lat, lon=self.lon).to_SHC(self.lmax)
        I_20C = GRD(grid=CoreI20C, lat=self.lat, lon=self.lon).to_SHC(self.lmax)
        I_21C = GRD(grid=CoreI21C, lat=self.lat, lon=self.lon).to_SHC(self.lmax)
    ...
    def G_Matrix_Term(self, mask=None):
        ...
        GRACE_SH[:, 5:8] = 0
        # GRACE_SH[:,12] = 0
        kernal = (SHC(c=GRACE_SH).to_GRD(self.res).value) * (self.setOcean(ocean_mask=mask))
    ...
    def GRD_Term(self, C10=None, C11=None, S11=None, C20=None, C21=None, S21=None, mask=None, SAL=False, rotation=True):
        GRACE_SH = self.GRACE.value
        GRACE_SH[:, 1] = S11
        GRACE_SH[:, 2] = C10
        GRACE_SH[:, 3] = C11
    ...
    def Low_Degree_Term(self, mask=None, SAL=False, rotation=True):
        """
    ...
    def GSM_Like(self, mask=None, SAL=False, rotation=True):
        SH = self.Low_Degree_Term(mask=mask, SAL=SAL, rotation=rotation)
        C = SH['Mass']
    ...
    def PM_mass_term(self, mask=None, SAL=False, rotation=True,isMas=False):
        SH = self.Low_Degree_Term(mask=mask, SAL=SAL, rotation=rotation)
        C = SH['EWH']
