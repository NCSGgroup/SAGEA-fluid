...
def demo_mask():
    ...
    np.save('D:/Cheung/SageaOcean/data/mask_ocean.npy',mask_ocean)
...
def demo_gfc():
    ...
    save_path = FileTool.get_project_dir("temp/slc.npz")
    np.savez(save_path, data=grid.value, lon=grid.lon, lat=grid.lat)
...
def demo1():
    ...
    filter_method, filter_params = Enums.SHCFilterType.DDK, (3,)
    leakage = Enums.LeakageMethod.Iterative
    # leakage = Enums.LeakageMethod.ForwardModeling
    """end of input"""
    ...
    # shc.add(shc_gac, lbegin=1)  # recovery of GAD model
